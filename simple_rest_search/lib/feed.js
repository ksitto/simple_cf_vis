...
function Feed(options) {
    ...
    this.addItem = function (options) {
        /* Not sure why we process options here. Validation occurs later on when we go to render, this only makes it harder to render additional fields
	options = options || {};
        var item = {
            title:          options.title,
            link:           options.link,
...
function atom_1_0(options) {
	...
	// classification
	if(entries[i].classification == undefined)
		throw new Error('entry need a classification');
	entry.push({ classification: entries[i].classification });

	if(entries[i].ownerProducer == undefined)
		throw new Error('entry need an ownerProducer');
	entry.push({ ownerProducer: entries[i].ownerProducer });

	// georess
	if(entries[i].point)
		entry.push({ point: entries[i].point });
